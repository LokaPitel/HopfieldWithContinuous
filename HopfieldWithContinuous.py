...
def print_state(state, prev_state):
    for i in range(state.shape[0]):
        for j in range(state.shape[1]):
            ...
            if np.round(value, 3) == np.round((prev_state[i, j]), 3):
                if (value > 0):
                    print(fg('sea_green_1b') + "{0: 5.1f}".format(value) + attr('reset'), end='')

                else:
                    print(fg('deep_pink_2') + "{0: 5.1f}".format(value) + attr('reset'), end='')

            else:
                print(fg('yellow_1') + "{0: 5.1f}".format(value) + attr('reset'), end='')
...
if __name__ == '__main__':
    with open('image.txt') as image:
        ...
        while True:
            ...
            state = activation(state @ W_matrix)
